...
$and_tempo = "";
if($tempo == 1){
    $and_tempo = "AND IF(gi.timemodified  IS NOT NULL, FROM_UNIXTIME(gi.timemodified) >= DATE_SUB(NOW(), INTERVAL 2 WEEK), FROM_UNIXTIME(gi.timecreated) >= DATE_SUB(NOW(), INTERVAL 2 WEEK))";
}elseif($tempo == 2){
    $and_tempo = "AND IF(gi.timemodified  IS NOT NULL, FROM_UNIXTIME(gi.timemodified) >= DATE_SUB(NOW(), INTERVAL 1 MONTH), FROM_UNIXTIME(gi.timecreated) >= DATE_SUB(NOW(), INTERVAL 1 MONTH))";
}elseif($tempo == 3){
    $and_tempo = "AND IF(gi.timemodified  IS NOT NULL, FROM_UNIXTIME(gi.timemodified) >= DATE_SUB(NOW(), INTERVAL 2 MONTH), FROM_UNIXTIME(gi.timecreated) >= DATE_SUB(NOW(), INTERVAL 2 MONTH))";
}
